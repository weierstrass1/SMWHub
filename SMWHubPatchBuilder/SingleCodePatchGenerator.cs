...
public static partial class SingleCodePatchGenerator
{
    ...
    public static string? GenerateSingleCode(int id, Code code)
    {
        ...
        string patchPath = Path.Combine(SINGLE_CODE_PATCH_DIRECTORY, 
            code.Scope.SourceDirectoryPath, $"Patch{code.FilePath}");
        Directory.CreateDirectory(Path.GetDirectoryName(patchPath)!);
        File.WriteAllText(patchPath, sb.ToString());
        File.Copy(code.SourcePath, Path.Combine(SINGLE_CODE_PATCH_DIRECTORY, code.ScopePath), true);
        var relatedCodes = code.GetRelatedCodes();
        string relatedCodePath;
        foreach (var relatedCode in relatedCodes) 
        {
            relatedCodePath = Path.Combine(SINGLE_CODE_PATCH_DIRECTORY, relatedCode.ScopePath);
            Directory.CreateDirectory(Path.GetDirectoryName(relatedCodePath)!);
            File.Copy(relatedCode.SourcePath, relatedCodePath, true);
        }
        return patchPath;
    }
        ...
        foreach (CodeLine line in code.ReadLines())
        {
            ...
            foreach (Match m in matches)
            {
                if (m.Groups["namespace"].Success)
                    namespaceCounter++;
                else if (m.Groups["off"].Success)
                    namespaceCounter--;
                else if (namespaceCounter == 0 && !labels.Contains(m.Value[..^1]))
                {
